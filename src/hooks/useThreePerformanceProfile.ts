...
const DESKTOP: ThreePerformanceProfile = {
  ...
  maxPointCloudPoints: 14_000,
  meshMaxSourcePoints: 3000,
  dpr: [1, 2],
...
const MOBILE: ThreePerformanceProfile = {
  ...
  maxPointCloudPoints: 4800,
  meshMaxSourcePoints: 1700,
  dpr: [1, 1.2],
...
function computeFromWindow(): ThreePerformanceProfile {
  ...
  if (tiny) {
    return {
      ...MOBILE,
      maxPointCloudPoints: 3600,
      meshMaxSourcePoints: 1400,
...
/** Profilo sincrono (stessa logica dell’hook) per downsample / opzioni ricostruzione fuori da React. */
