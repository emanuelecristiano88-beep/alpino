...
export type DepthToCloudParams = {
  ...
  sampleStep: number;
  /**
   * Se definito, sostituisce `transformPointByPhase` (es. pipeline stable con angoli per zona).
   */
...
export function depthMapToPointCloud(params: DepthToCloudParams): {
  ...
  const { depth01, mask, imageData, phaseId, depthNearMm, depthFarMm, focalLengthNorm, sampleStep, transformPoint } =
    params;
  ...
  for (let y = 0; y < h; y += step) {
    for (let x = 0; x < w; x += step) {
      ...
      const pw = transformPoint ? transformPoint(p0) : transformPointByPhase(p0, phaseId);
