...
/** Opzioni `regularizeFootPointCloud`: picchi, profondità, smoothing superficie. */
export type FootShapeRegularizeOptions = {
  spikeClamp: boolean;
  spikeSearchRadiusMm: number;
  spikeMaxNeighbors: number;
  spikeMaxDeviationFromDiagonal: number;
  spikeMinClampMm: number;
  depthClamp: boolean;
  depthSearchRadiusMm: number;
  depthMaxNeighbors: number;
  thinAxis: "auto" | 0 | 1 | 2;
  maxDepthDeviationMm: number;
  maxDepthDeviationRelativeToThinExtent: number;
  maxDepthDeviationCapMm: number;
  surfaceSmoothPasses: number;
  surfaceSmoothRadiusMm: number;
  surfaceSmoothK: number;
  surfaceSmoothBlend: number;
};
...
export type ReconstructionOptions = {
  ...
  /**
   * Se true (default), i frame con lo stesso `phaseId` vanno mediati (voxel → centroide) prima
   * dell’allineamento multi-vista. `multiViewReferenceCloudIndex` si riferisce all’elenco
   * nuvole **dopo** questa fusione, ordinate per fase 0→3.
   */
  intraZoneFrameAverage?: boolean;
  /**
   * Lato voxel (mm) per la media intra-zona. Se omesso: `min(3, voxelSizeMm * 0.55)`.
   */
  intraZoneFrameVoxelMm?: number;
  /**
   * Se true (default), il merge voxel tra zone usa pesi per fase (`SCAN_PHASE_MERGE_WEIGHT`).
   */
  phaseWeightedMerge?: boolean;
  /** Regolarizza forma (picchi, profondità, smoothing) dopo merge. Default: false (pipeline veloce). */
...
export type ReconstructionResult = {
  ...
  meta: {
    ...
    multiViewRefinementIterations?: number;
    /** Punti per frame in ingresso (prima della media intra-zona) */
    pointsPerInputFrame?: number[];
    /** Media intra-zona applicata (burst → una nuvola per fase) */
    intraZoneFrameAverageApplied?: boolean;
    /** Numero di nuvole dopo la media intra-zona (≤ 4, una per fase) */
    zoneCloudCount?: number;
    /** Ordine fasi delle nuvole in `pointsPerView` post-fusione */
    zonePhaseIds?: ScanPhaseId[];
    /** Merge voxel con pesi per zona (INNER ↑, HEEL/OUTER ↓) */
